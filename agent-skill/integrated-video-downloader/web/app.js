...
const SERVICE_URL = ["127.0.0.1", "localhost"].includes(location.hostname) ? location.origin : "http://127.0.0.1:8765";
...
elements.directActionButton = document.querySelector("#directActionButton");
elements.directActionText = document.querySelector("#directActionText");
elements.serviceHelp = document.querySelector("#serviceHelp");
elements.statusDot = document.querySelector("#statusDot");
...
function detectPlatform(value) {
  ...
  try {
    ...
    if (host.endsWith(".weibocdn.com") || host.endsWith(".sinaimg.cn")) return "weibo";
...
function render() {
  ...
  elements.directActionText.textContent = task.action === "resolve" ? "开始解析" : "直接下载";
  drawSignal(task);
}
...
function filenameFromDisposition(value) {
  if (!value) return "video-download";
  const encoded = value.match(/filename\*=UTF-8''([^;]+)/i);
  if (encoded) return decodeURIComponent(encoded[1]);
  const plain = value.match(/filename="?([^";]+)"?/i);
  return plain ? plain[1] : "video-download";
}

async function checkService() {
  try {
    const response = await fetch(`${SERVICE_URL}/api/status`, {signal: AbortSignal.timeout(1500)});
    const data = await response.json();
    serviceOnline = response.ok && data.ok;
  } catch (_) {
    serviceOnline = false;
  }
  elements.statusDot.classList.toggle("offline", !serviceOnline);
  elements.statusText.textContent = serviceOnline ? "下载服务已连接" : "下载服务未连接";
  if (serviceOnline) elements.serviceHelp.hidden = true;
}
...
async function runDirectAction() {
  ...
  if (!task) return;
  if (!serviceOnline) {
    elements.serviceHelp.hidden = false;
    showToast("请先启动本地下载服务");
    return;
  }
  elements.directActionButton.disabled = true;
  const original = elements.directActionText.textContent;
  elements.directActionText.textContent = task.action === "resolve" ? "解析中..." : "下载中...";
  try {
    const endpoint = task.action === "resolve" ? "resolve" : "download";
    ...
    if (!response.ok || contentType.includes("application/json")) {
      const data = await response.json();
      if (!data.ok) throw new Error(data.error || "下载服务返回错误");
      elements.resultInput.value = JSON.stringify(data, null, 2);
      renderResult();
    ...
    elements.directActionButton.disabled = false;
    elements.directActionText.textContent = original;
  }
}
...
elements.directActionButton.addEventListener("click", runDirectAction);
document.querySelector("#downloadTaskButton").addEventListener("click", downloadTask);
document.querySelector("#copyServerCommand").addEventListener("click", () => copyText(document.querySelector("#serverCommand").textContent, "启动命令已复制"));
